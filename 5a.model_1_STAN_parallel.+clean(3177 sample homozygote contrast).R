...
# Source file: 5a.model_1_STAN_parallel.+clean(3177 sample homozygote contrast).R
# Pipeline process 1/12: Model 1 fitting in the primary 3,177-sample homozygote contrast
#
# Purpose: Prepare the primary APOE homozygote longitudinal data set and fit per-outcome
# Bayesian spline models comparing APOE e4/e4 to APOE e3/e3 across EYO.
#
# Input:  APOE_homozygote_2ndFinal_cleanDats+traits(3177.CT33only_4199.33inADcontinuum).RData
# Input:  cleanDat.3177, MEs.3177, numericMeta.3177
# Output: simple.3177/*_stan_glm.rds
# Output: simple.3177/name_match_table.RDS
# Output: _numericMeta_3177_trait.RDS
# Output: _full_3177_protein_dft.RDS
#
# Major analysis steps in this script:
#   1. Combine assay-level abundances with WGCNA module eigengenes.
#   2. Remove assay outliers >5 SD from the assay mean and enforce <=20% missingness per APOE
#      group.
#   3. Append MMSE and CDR outcomes and align sample IDs between metadata and outcome matrix.
#   4. Define ApoE_Indicator and EYO = age_at_visit - 65.6.
#   5. Fit Model 1 independently for every outcome using rstanarm::stan_glm in a
#      doParallel/foreach loop.
#
...
# ----------------------------------------------------------------------------------------
# Set the working directory and load packages for primary Model 1 fitting.
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# Load the primary/sensitivity APOE homozygote workspace generated upstream.
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# Build the outcome matrix by combining protein assays and module eigengenes.
# ----------------------------------------------------------------------------------------
full_3177_protein_df <- as.data.frame(rbind(cleanDat.3177, t(MEs.3177)))


# ----------------------------------------------------------------------------------------
# Apply 5-SD outlier removal before modeling.
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# Index samples by APOE genotype to enforce the missingness rule within each group.
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# Define the binary APOE e4/e4 indicator used in all longitudinal models.
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# Define estimated years from onset (EYO) as age_at_visit minus 65.6.
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# Utility: sanitize assay names into syntactically safe model/output names.
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# Move sample_id to the first column for joins with traits.
# ----------------------------------------------------------------------------------------



# ----------------------------------------------------------------------------------------
# Define the outcome list for per-assay/per-outcome Stan modeling.
# ----------------------------------------------------------------------------------------
pep_names <- names(protein_df)[2:dim(protein_df)[2] ]


# ----------------------------------------------------------------------------------------
# Worker function: assemble the complete-case analysis data and fit one outcome.
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# Create the restricted cubic spline basis for EYO.
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# Model 1 formula: EYO spline terms, APOE group, and APOE-by-EYO spline interactions.
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# Fit the Bayesian Gaussian regression model with rstanarm.
# ----------------------------------------------------------------------------------------
  ...
  fil_stan_1 <- file.path("z:/","ShijiaBian","PlasmaProteomic","Result","20250727", "simple.3177", paste(paste(outcome, "_stan_glm", ".rds", sep = "")))


# ----------------------------------------------------------------------------------------
# Strip formula environments to keep saved RDS files compact.
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# Start the parallel backend for per-outcome model fitting.
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# Run Model 1 across all outcomes.
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# Save name matching and cleaned input data for plotting scripts.
# ----------------------------------------------------------------------------------------
saveRDS(name_match_table,"z:/ShijiaBian/PlasmaProteomic/Result/20250727/simple.3177/name_match_table.RDS")
saveRDS(numericMeta_3177_trait,"_numericMeta_3177_trait.RDS")
saveRDS(protein_df,"_full_3177_protein_dft.RDS")
