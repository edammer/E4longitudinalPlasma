...
# Source file: 5a2.Plot_STAN_model_1_20250718-direct2PDF+parallel(3177) _5xSDcleaned-0727.R
# Pipeline process 2/12: Model 1 trajectory plotting and posterior tail matrices in the primary cohort
#
# Purpose: Post-process the primary-cohort Model 1 fits into e3/e3 and e4/e4 EYO trajectories,
# posterior differences, PDF plots, and EYO-by-assay significance matrices.
#
...
# Output: simple.3177/scatter/99par_2on1_*.pdf
# Output: simple.3177/scatter/_99_par_diff_all_peptide.rds/.csv
# Output: simple.3177/scatter/_99_par_diff_all_peptide_p_value.rds/.csv
# Output: simple.3177/scatter/_99_par_diff_all_peptide_up_down_notation.csv
#
# Major analysis steps in this script:
#   1. Load saved Model 1 fits and recover posterior draws.
#   2. Evaluate genotype-specific restricted-cubic-spline trajectories on a 0.5-year EYO grid.
#   3. Compute posterior e4/e4 minus e3/e3 differences and 99% posterior intervals.
#   4. Draw combined trajectory and difference plots for each protein/outcome.
#   5. Collect posterior tail probabilities, signed effects, and up/down notations for downstream
#      Section 6 analyses.
#
...
# ----------------------------------------------------------------------------------------
# Load plotting, posterior extraction, and Stan support packages.
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# Load primary-cohort trait and outcome matrices produced by 5a.
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# Load the model-name lookup table and plot labels.
# ----------------------------------------------------------------------------------------
...
pep_names <- names(BL_traits_pep)[c(2:7348)]  #12 modules at end, then MMSE and cdr
...
# ----------------------------------------------------------------------------------------
# Source the custom trajectory and difference plotting functions.
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# Worker function: read one saved Model 1 fit and compute trajectory summaries.
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# Extract posterior draws from the saved rstanarm model.
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# Create the half-year EYO grid for plotting and matrix output.
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# Evaluate the restricted-cubic-spline basis at each EYO grid point.
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# Use posterior draws to compute e3/e3, e4/e4, and difference trajectories.
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# Summarize 99% posterior intervals and posterior tail probabilities at each EYO point.
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# Prepare observed sample-level points for overlay on model curves.
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# Draw observed data and modeled genotype-specific trajectories.
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# Prepare the posterior difference data frame for the second panel.
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# Draw the posterior difference curve and interval.
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# Combine trajectory and difference panels into one PDF page.
# ----------------------------------------------------------------------------------------
...
#        ggsave(filename = paste0("z:/ShijiaBian/PlasmaProteomic/Result/20250727/simple.3177/scatter/99par_2on1_",gsub("(\\|)|(\\.)|(\\;)", "_",pep_name),".pdf"), plot = on_same_plot, device = cairo_pdf,
        ggsave(filename = paste0("F:/OneDrive - Emory/Legacy/e4_homozygoteStudy/DL/simple.3177/scatter/99par_2on1_",gsub("(\\|)|(\\.)|(\\;)", "_",pep_name),".pdf"), plot = on_same_plot, device = cairo_pdf,
...
# ----------------------------------------------------------------------------------------
# Return per-outcome p-value, effect-size, and direction summaries to the parallel
# collector.
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# Assemble EYO-by-outcome matrices from successful workers.
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# Save posterior median/t-statistic-like difference matrices.
# ----------------------------------------------------------------------------------------
...
fil_pep<-"./simple.3177/scatter/_99_par_diff_all_peptide.rds"
...
fil_csv_pep <- "./simple.3177/scatter/_99_par_diff_all_peptide.csv"
...
# ----------------------------------------------------------------------------------------
# Save posterior tail probability matrix.
# ----------------------------------------------------------------------------------------
fil_pep<-"./simple.3177/scatter/_99_par_diff_all_peptide_p_value.rds"
...
fil_csv_pep <- "./simple.3177/scatter/_99_par_diff_all_peptide_p_value.csv"
write.csv(noncarrier_carrier_p_value_all_pep_final, fil_csv_pep, row.names = TRUE)


# ----------------------------------------------------------------------------------------
# Save up/down direction notation for downstream enrichment.
# ----------------------------------------------------------------------------------------
...
fil_csv_up_down_notation<-"./simple.3177/scatter/_99_par_diff_all_peptide_up_down_notation.csv"
