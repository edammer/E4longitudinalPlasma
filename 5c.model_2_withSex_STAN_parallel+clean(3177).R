...
# Source file: 5c.model_2_withSex_STAN_parallel+clean(3177).R
# Pipeline process 5/12: Models 2.1 and 2.2 fitting in the primary cohort
#
# Purpose: Fit sex-adjusted and sex-by-EYO interaction models for each outcome in the
# 3,177-sample primary cohort.
#
...
# Output: sexInt.3177/*_with_Sex_stan_glm.rds
# Output: sexInt.3177/*_with_Sex_Interaction_stan_glm.rds
# Output: sexInt.3177/name_match_table.RDS
#
# Major analysis steps in this script:
#   1. Repeat primary-cohort cleaning and trait derivation.
#   2. Merge each outcome with ApoE_Indicator, EYO, and Sex.int.
#   3. Fit Model 2.1 with APOE-by-EYO terms plus sex main effect.
#   4. Fit Model 2.2 by adding sex-by-EYO spline terms.
#   5. Save both model families with stripped formula environments for compact storage.
#
...
# ----------------------------------------------------------------------------------------
# Set the working directory and load packages for primary-cohort sex-interaction modeling.
# ----------------------------------------------------------------------------------------
setwd("z:/ShijiaBian/PlasmaProteomic/Result/20250727/sexInt.3177/")
library(tidyverse)
library(forcats)
library(rstanarm)
library(Hmisc)
...
# ----------------------------------------------------------------------------------------
# Build the outcome matrix from proteins and module eigengenes.
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# Utility: sanitize names for model outputs.
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# Worker function for fitting Model 2.1 and Model 2.2 for one outcome.
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# Join trait covariates, including Sex.int, to the outcome.
# ----------------------------------------------------------------------------------------
  dat <- traits_df %>%
...
# ----------------------------------------------------------------------------------------
# Generate EYO spline basis terms.
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# Model 2.1: Model 1 plus a sex main effect.
# ----------------------------------------------------------------------------------------
  ...
  fil_stan_1 <- file.path("z:/","ShijiaBian","PlasmaProteomic","Result","20250727", "sexInt.3177", paste(paste(outcome, "_with_Sex_stan_glm", ".rds", sep = "")))


# ----------------------------------------------------------------------------------------
# Model 2.2: add sex-by-EYO spline interactions.
# ----------------------------------------------------------------------------------------
  ...
  fil_stan_2 <- file.path("z:/","ShijiaBian","PlasmaProteomic","Result","20250727", "sexInt.3177", paste(paste(outcome, "_with_Sex_Interaction_stan_glm", ".rds", sep = "")))


# ----------------------------------------------------------------------------------------
# Strip formula environments from both saved models.
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# Fit Models 2.1 and 2.2 across outcomes.
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# Save the model-name lookup table.
# ----------------------------------------------------------------------------------------

saveRDS(name_match_table,"z:/ShijiaBian/PlasmaProteomic/Result/20250727/sexInt.3177/name_match_table.RDS")
