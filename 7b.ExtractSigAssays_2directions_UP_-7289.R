## =============================================================================
## 7b. ExtractSigAssays_2directions_UP - sliding-window
##     significant-assay extraction (UP direction only) and
...
##   to those whose Model 1 posterior trajectory is up-regulated in
##   APOE e4/e4 vs e3/e3 (color-coded "indianred3" in the up/down direction
##   matrix from 5a2/5e2). See 7a's header for the full shared pipeline
##   description; only UP-specific differences are called out below.
##
## KEY DIFFERENCES FROM 7a (ALL)
##   - The per-window gene list fed to the enrichment engine is
##     collect_sigUP_by_window(), i.e. up-only assays.
...
##     plots UP-only Z-scores for those SAME 141 terms, so that the
...
##
## MAJOR OUTPUTS
##   - ./SEPA.all_redelivered/UP_assay-Zscore_trajectories.pdf /
##     ..._29_SELECTED_1pp.pdf
##   - ./SEPA.all_redelivered/UP_assaysONLY-Z_GO-heatmaps_rowsOrdered_top100.pdf /
##     ..._29_SELECTED_1pp.pdf
##   - .../3.SEPAwindows(67)UP(top100)_18categoriesSeparated-Z_GO-heatmaps_rowsOrdered.pdf
##     (final 18-category, 141-term UP-only publication heatmap)
##   - ./SEPA.all_redelivered/UP-141Heatmap-ordered_ontologies+Zscores.csv
##   - ./SEPA.all_redelivered/UP-Genes_Hit(141selectedOntologies_67_5yrWindows.csv
##   - ./SEPA.all_redelivered/saved.image-67_5yearInterval_slidingWindow_GOparallel.UPhitsONLY-complete-7289_redelivered.RData
## =============================================================================
...
#-43.5 -42.5 -41.5 -40.5 -39.5 -38.5 -37.5 -36.5 -35.5 -34.5 -33.5 -32.5 -31.5 -30.5 -29.5 -28.5 -27.5 -26.5 -25.5 -24.5 -23.5 -22.5 -21.5 -20.5 -19.5 -18.5 -17.5 -16.5 -15.5 -14.5 -13.5 -12.5 -11.5 -10.5  -9.5  -8.5  -7.5  -6.5 
#  164   164   165   168   168   172   173   173   180   178   181   180   182   177   179   182   181   185   187   191   200   200   207   218   231   236   248   259   271   285   298   315   332   344   345   347   359   368 
# -5.5  -4.5  -3.5  -2.5  -1.5  -0.5   0.5   1.5   2.5   3.5   4.5   5.5   6.5   7.5   8.5   9.5  10.5  11.5  12.5  13.5  14.5  15.5  16.5  17.5  18.5  19.5  20.5  21.5  22.5 
#  372   381   389   396   405   411   424   449   480   507   544   572   588   599   600   587   556   522   473   425   364   325   283   260   243   227   209   195   182
...
## Current
#-43.5 -42.5 -41.5 -40.5 -39.5 -38.5 -37.5 -36.5 -35.5 -34.5 -33.5 -32.5 -31.5 -30.5 -29.5 -28.5 -27.5 -26.5 -25.5 -24.5 -23.5 -22.5 -21.5 -20.5 -19.5 -18.5 -17.5 -16.5 -15.5 -14.5 -13.5 -12.5 -11.5 -10.5  -9.5  -8.5  -7.5  -6.5 
#   71    70    70    70    67    67    68    67    69    68    68    66    68    69    69    69    67    68    69    73    76    76    78    83    89    93   102   112   117   122   130   140   149   157   159   162   171   177 
# -5.5  -4.5  -3.5  -2.5  -1.5  -0.5   0.5   1.5   2.5   3.5   4.5   5.5   6.5   7.5   8.5   9.5  10.5  11.5  12.5  13.5  14.5  15.5  16.5  17.5  18.5  19.5  20.5  21.5  22.5 
#  182   188   199   202   211   214   221   239   260   279   304   324   330   344   349   341   322   299   269   245   204   183   154   142   138   129   119   112   103
...
## Current
#-43.5 -42.5 -41.5 -40.5 -39.5 -38.5 -37.5 -36.5 -35.5 -34.5 -33.5 -32.5 -31.5 -30.5 -29.5 -28.5 -27.5 -26.5 -25.5 -24.5 -23.5 -22.5 -21.5 -20.5 -19.5 -18.5 -17.5 -16.5 -15.5 -14.5 -13.5 -12.5 -11.5 -10.5  -9.5  -8.5  -7.5  -6.5 
#   98   100   100   103   106   108   109   108   112   115   114   116   117   111   113   115   116   122   122   121   126   129   133   136   144   146   152   154   157   164   169   177   184   190   190   188   189   194 
# -5.5  -4.5  -3.5  -2.5  -1.5  -0.5   0.5   1.5   2.5   3.5   4.5   5.5   6.5   7.5   8.5   9.5  10.5  11.5  12.5  13.5  14.5  15.5  16.5  17.5  18.5  19.5  20.5  21.5  22.5 
#  194   194   192   196   195   199   204   212   227   231   243   253   260   260   254   247   235   226   206   180   163   143   131   120   105    99    94    88    84
...
# using each window's UP-only significant-assay list as the test set. ----
##############################
...
outFilename <- "SEPA.up"  #SUBFOLDER WITH THIS NAME WILL BE CREATED, and .PDF + .csv file using the same name will be created within this folder.
...
#	            stop("argument gsc should be of class GSC, as returned by the loadGSC function")  # disabled since the list we create is not of GSC class
	...
	# STEP 5 (continued): "uniquemodcolors" holds the 67 EYO sliding-window
	# names, and "modulesData" holds each window's UP-only
	# significant-assay list -> one hypergeometric FET enrichment is run per
	# EYO window, in parallel, against the full GMT gene-set background.
	...
	#  this.geneList=uniquemodcolors[i]
	  ...
	  # ---- STEP 6. Recombine per-window FET results across all 67 EYO
	  # windows, compute signed Z-scores per ontology term, and assemble
	  # Zout.list (one terms x 67-window Z-score matrix per ontology
	  # category) for the UP-only significant-assay lists. ----
...
save.image("./SEPA.all_redelivered/saved.image-67_5yearInterval_slidingWindow_GOparallel.UPhitsONLY-complete-7289_redelivered.RData")


# ---- STEP 7. Plot smoothed Z-score trajectory line plots and row-clustered
# enrichment heatmaps for the top-ranked UP-only terms in each
...
pdf("./SEPA.all_redelivered/UP_assay-Zscore_trajectories.pdf", width = 14, height = 8, onefile = TRUE)
for (nm in names(Zout.list)) {
  df <- Zout.list[[nm]]
  if (!is.null(df) && nrow(df) > 0 && ncol(df) > 0) {
    print( plot_Zout_df(df, title = paste0("UP assay Z-score trajectories: ", nm), span = 0.5) )
  }
}
dev.off()
...
pdf("./SEPA.all_redelivered/UP_assaysONLY-Z_GO-heatmaps_rowsOrdered_top100.pdf", width = 17.5, height = 35, onefile = TRUE)
#grid::grid.newpage()
#par(mar=c(1,1,1,15))
#par(oma=c(1,1,1,30))
for (nm in names(Zout.list)) {
  df <- Zout.list[[nm]]
  if (!is.null(df) && nrow(df) > 0 && ncol(df) > 0) {
    plot_Zout_heatmap(df, title = paste0("UP ASSAYS Z-score heatmap: ", nm))
  }
}
dev.off()
...
pdf("./SEPA.all_redelivered/UP_assay-Zscore_trajectories_29_SELECTED_1pp.pdf", width = 14, height = 8, onefile = TRUE)
  df <- Zout.selected
  if (!is.null(df) && nrow(df) > 0 && ncol(df) > 0) {
    print( plot_Zout_df(df, title = paste0("UP assay Z-score trajectories: ", "Selected Terms"), span = 0.5) )
  }
dev.off()

# Replot same 20/29 selected terms as heatmap
pdf("./SEPA.all_redelivered/UP_assaysONLY-Z_GO-heatmaps_rowsOrdered_29_SELECTED_1pp.pdf", width = 17.5, height = 8, onefile = TRUE)
  if (!is.null(df) && nrow(df) > 0 && ncol(df) > 0) {
    plot_Zout_heatmap(df, title = paste0("UP ASSAYS Z-score heatmap: ", "Selected Terms"))
  }
dev.off()
...
plot_Zout_heatmap2 <- function(mat, title=NULL, thr=1.645, rn = rownames(mat), draw=TRUE, cols=NULL, breaks=NULL, reorderRows=TRUE, ...) {
  ...
  if (is.null(cols) | is.null(breaks)) {
    ...
    if (maxZ <= thr) {
      ...
      cols   <- c("white", "#CDCDCD", "#CDCDCD", colorRampPalette(c("#CD5555", "yellow"))(n_grad))
    }
  }
...
# plot red-black heatmap
pdf("F:/OneDrive - Emory/Legacy/e4_homozygoteStudy/DL/3.Five_yr_slidingWindow/SEPA.all_redelivered/3.SEPAwindows(67)UP(top100)_assays-Z_GO-heatmaps_rowsOrdered_66_SELECTED_1pp.pdf", width = 17.5, height = 14, onefile = TRUE)
  df <- Zout.selected2
  rownames(df) <- sub("^([^\\.]+)\\.(.*)$", "\\2 (\\1)", rownames(df))
  if (!is.null(df) && nrow(df) > 0 && ncol(df) > 0) {
    plot_Zout_heatmap2(df, title = paste0("UP ASSAYS Z-score heatmap: ", "66 Selected Terms"))
  }
dev.off()
...
if (global_maxZ <= thr) {
  ...
  n_grad <- 100L
  breaks <- c(minBreak, thr, seq(thr, global_maxZ, length.out = n_grad + 1L)[-1L])
  cols   <- c("black", colorRampPalette(c("#8B0000", "yellow"))(n_grad))
}
...
pdf("F:/OneDrive - Emory/Legacy/e4_homozygoteStudy/DL/3.Five_yr_slidingWindow/SEPA.all_redelivered/3.SEPAwindows(67)UP(top100)_XXcategoriesSeparated(XXXassays)-Z_GO-heatmaps_rowsOrdered.pdf", width = 20, height = 30, onefile = TRUE)
...
# SAME terms using only this script's UP-direction enrichment
# results, so the ALL/UP/DOWN publication heatmaps are term-for-term
# comparable. ----
## GO back and curate the same 141 terms from ALL (UP+DOWN) but Z scores are from UP hits only.
...
if (global_maxZ <= thr) {
  ...
  cols   <- c("#FFFFFF", "#CDCDCD", "#CDCDCD", colorRampPalette(c("#CD5555", "yellow"))(n_grad))
}
...
# ---- STEP 9. Draw the final, curated 18-category / 141-term UP-only
# publication heatmap using the fixed FinalOrder2 category/term table. ----
## Heatmap rendered in final row order below 141x67


##############################################################################################
# ---- STEP 10. Write the final 141x67 UP-only Z-score matrix and
...
pdf("F:/OneDrive - Emory/Legacy/e4_homozygoteStudy/DL/3.Five_yr_slidingWindow/SEPA.all_redelivered/3.SEPAwindows(67)UP(141ALL)_18categoriesSeparated-Z_GO-heatmaps_rowsOrdered.pdf", width = 20, height = 30, onefile = TRUE)
  draw(ht_list, merge_legend = TRUE)
dev.off()
############################
...
dim(longDT.genesHit)
#[1] 247470      6  (current; 7289 assays)
#[1] 248004      6  (prior; 7333 assays)
...
write.csv(z.141[exactGO.141$NewAnnotation,],"./SEPA.all_redelivered/UP-141Heatmap-ordered_ontologies+Zscores.csv")
...
# Write genes hit in 141x67 matrix to CSV
write.csv(genesHit.141,"./SEPA.all_redelivered/UP-Genes_Hit(141selectedOntologies_67_5yrWindows.csv")
...
## Down would be run as above for UP
