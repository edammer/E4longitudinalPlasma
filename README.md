...
Tests whether the proteins identified as significant in each 5-year EYO sliding window (and in a simple before-/after-age-50 split) are enriched for proteins previously reported as associated with neuropathology or cognitive-function endophenotypes (Nat Aging cohort marker lists), using two-sided Fisher's Exact Test (FET) overlap enrichment via the lab's general-purpose `geneListFET()` wrapper. Strips non-protein `seq.####` control-assay entries from the sliding-window hit list before testing.
